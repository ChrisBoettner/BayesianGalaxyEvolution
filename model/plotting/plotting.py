...
from model.analysis.reference_parametrization import (
    get_reference_function_sample,
...
from model.plotting.convience_functions import (
    plot_group_data,
    plot_best_fit_ndf,
...
class Plot(ABC):
    ...
    def to_print_mode(
        self, labelsize_scaling=1, leftbottom_ticks=True, labels_off=False
    ):
        """
    ...
    def save(self, file_format="pdf", file_name=None, path=None, print_mode=False):
        """
        ...
        self.fig.savefig(path + file, format=file_format, transparent=transparent)
        return self
...
class Plot_best_fit_ndf(Plot):
    ...
    def _plot(self, ModelResults, datapoints=True, redshift=0):
        ...
        if ModelResults[0].parameter.is_None():
            raise AttributeError("best fit parameter have not been calculated.")
        ...
        xlabel, ylabel = (
            ModelResults[0].quantity_options["ndf_xlabel"],
...
class Plot_best_fit_ndfs_all_z(Plot):
    ...
    def _plot(self, ModelResults, datapoints=True):
        ...
        # add legend
        if (len(ModelResults) == 1) and (ModelResults[0].physics_name == "changing"):
            separation_point = 2
        else:
            separation_point = len(ModelResults)
        add_separated_legend(
            axes, separation_point=separation_point, ncol=ncol, loc=legend_loc
        )
...
class Plot_marginal_pdfs(Plot):
    ...
    def _plot(self, ModelResults):
        ...
        fig, axes = plt.subplots(
            ModelResults[0].quantity_options["model_param_num"],
...
class Plot_parameter_sample(Plot):
    ...
    def _plot(
        ...
        # general plotting configuration
        param_num = ModelResult.distribution.at_z(ModelResult.redshift[0]).shape[1]
        ...
        if param_num > ModelResult.quantity_options["model_param_num"]:
            m_c_label = (
                r"$\log M_\mathrm{c}^"
                + ModelResult.quantity_options["quantity_subscript"]
                + r"$"
            )
        ...
        cm = LinearSegmentedColormap.from_list(
            "Custom", [washed_out_color, self.color], N=num
        )

        # check if log_m_c is kept free at any redshift
        free_m_c = False in [
            ModelResult.physics_model.at_z(z).fixed_m_c_flag
        ...
        for z in ModelResult.redshift:
            ...
            parameter_sample, color = sort_by_density(parameter_sample, keep=cols)
            ...
            if z >= marg_z:
                ...
                for i, j in enumerate(ModelResult.physics_model.at_z(z).parameter_used):
                    if free_m_c:
                        # put log_m_c on last axis
                        k = j - 1 if j > 0 else parameter_sample.shape[-1] - 1
                        axes[k].scatter(
        ...
        for i, ax in enumerate(axes.flatten()):
            ...
            upper_lim = 1.15 * curr_lim[1] if curr_lim[1] > 0 else 0.85 * curr_lim[1]
...
class Plot_qhmr(Plot):
    ...
    def _plot(
        ...
        median=True,
        only_data=False,
        m_halo_range=np.linspace(10.7, 14.23, 1000),
        y_lims=None,
        legend=True,
    ):
        ...
        qhmr = {}
        for z in redshift:
            qhmr[z] = calculate_qhmr(
                ModelResult, z, sigma=sigma, ratio=ratio, log_m_halos=m_halo_range
            )
        ...
        cm = LinearSegmentedColormap.from_list(
            "Custom", ["lightgrey", self.color], N=len(redshift)
        )
...
class Plot_ndf_intervals(Plot):
    def __init__(self, ModelResult, **kwargs):
        ...
        by passing the (list of) model objects to additional_models, where the
        color can be determined by additional_model_color.
        """
        super().__init__(ModelResult, **kwargs)
    ...
    def _plot(
        ...
        best_fit=False,
        datapoints=True,
        feedback_regimes=True,
        additional_color="#ccc979",
        ...
        ndfs = {}
        for z in ModelResult.redshift:
            ndfs[z] = calculate_ndf_percentiles(ModelResult, z, sigma=sigma, num=num)
        ...
        # quantity specific settings
        xlabel, ylabel, ncol = (
            ModelResult.quantity_options["ndf_xlabel"],
        ...
        for z in ModelResult.redshift:
            plot_data_with_confidence_intervals(
                axes[z], ndfs[z], color, median=False, alpha=1
            )
        ...
        if feedback_regimes:
            plot_feedback_regimes(axes, ModelResult, color=additional_color, alpha=0.25)
        ...
        if additional_models:
            ...
            for model in models:
                plot_best_fit_ndf(
                    axes, model, linewidth=4, alpha=0.6, color=additional_model_color
                )
...
class Plot_ndf_predictions(Plot):
    ...
    def _plot(
        ...
        # make redshift space
        redshift = range(
            ModelResult.quantity_options["extrapolation_z"][-1], upper_redshift + 1
        )
        # get ndf sample
        ...
        for z in redshift:
            if z > redshift[0]:
                ndfs_fixed[z] = calculate_ndf_percentiles(
                    ModelResult,
                    redshift[0],
                    sigma=sigma,
                    num=num,
            ...
            ndfs_extrapolate[z] = calculate_ndf_percentiles(
                ModelResult,
                z,
                sigma=sigma,
                num=num,
        ...
        for i, z in enumerate(redshift):
            ...
            if z > redshift[0]:
                plot_data_with_confidence_intervals(
                    axes[i], ndfs_fixed[z], "lightgrey", median=False, alpha=0.7
                )
            plot_data_with_confidence_intervals(
                axes[i], ndfs_extrapolate[z], color, median=False, alpha=0.7
            )
...
class Plot_reference_function_sample(Plot):
    ...
    def _plot(self, ModelResult, datapoints=True):
        ...
        reference_functions = get_reference_function_sample(
            ModelResult, ModelResult.redshift
        )
        ...
        # define plot parameter
        plot_parameter = {"linewidth": 0.2, "alpha": 0.2, "color": "grey"}
        ...
        for z in ModelResult.redshift:
            for functions in reference_functions[z]:
                axes[z].plot(functions[:, 0], functions[:, 1], **plot_parameter)
        ...
        # add legend
        add_separated_legend(axes, separation_point=0, ncol=ncol, loc=legend_loc)
...
class Plot_reference_comparison(Plot):
    ...
    def _plot(self, ModelResults, datapoints=True):
        ...
        reference_data_params = calculate_reference_parameter_from_data(
            ModelResults[0], ModelResults[0].redshift
        )
        # calculate reference parameter for models
        reference_models = []
        for Model in ModelResults:
            reference_models.append(
                calculate_best_fit_reference_parameter(Model, Model.redshift)
            )
        ...
        plot_parameter_model = {"linewidth": 3, "alpha": 1, "color": "grey"}
        plot_parameter_reference = {"linewidth": 1.75, "alpha": 1, "color": "C2"}
        plot_parameter_ndf_sample = {"linewidth": 0.2, "alpha": 0.4, "color": "grey"}
        linestyle = ["-", "--", ":"]
        ...
        for i, Model in enumerate(ModelResults):
            ndfs = calculate_best_fit_ndf(Model, Model.redshift)
            ...
            for z in Model.redshift:
                ...
                label = (
                    Model.quantity_options["reference_function_name"]
                    + " Function ("
                    + Model.prior_name
                    + " prior)"
                )
        ...
        for z in ModelResults[0].redshift:
            axes[z].plot(
                quantity_range,
                reference_function(quantity_range, *reference_data_params[z]),
...
class Plot_q1_q2_relation(Plot):
    ...
    def make_plot(self, ModelResult1, ModelResult2, **kwargs):
        ...
        self.quantity_name = (
            ModelResult1.quantity_name + "_" + ModelResult2.quantity_name
        )
    ...
    def _plot(
        ...
        datapoints=False,
        scaled_ndf=None,
        scaled_ndf_color=None,
        quantity_range=None,
        log_slopes=None,
        log_slope_labels=None,
        masks=False,
        y_lims=None,
        legend=False,
        linewidth=5,
        ...
        if scaled_ndf_color:
            ...
            if len(scaled_ndf_color) != len(make_list(scaled_ndf[1])):
                raise ValueError(
                    "Length of scaled_ndf_alpha values must "
        ...
        q1_q2_relation = calculate_q1_q2_relation(
            ModelResult1, ModelResult2, z, log_q1, sigma=sigma, num=num, ratio=ratio
        )
        ...
        if scaled_ndf:
            if scaled_ndf[0] not in [ModelResult1, ModelResult2]:
                raise ValueError(
                    "Model for alternative ndf must be same as "
            ...
            for fudge_factor in ndf_fudge_factors:
                ...
                if AltModel.quantity_name == ModelResult1.quantity_name:
                    alt_relations[fudge_factor] = calculate_q1_q2_relation(
                        AltModel, ModelResult2, z, log_q1
                    )[1]
                elif AltModel.quantity_name == ModelResult2.quantity_name:
                    alt_relations[fudge_factor] = calculate_q1_q2_relation(
                        ModelResult1, AltModel, z, log_q1
                    )[1]
                else:
                    raise ValueError(
                        "AltModel quantity name does not match "
                        "either of the input ModelResults."
                    )

        ## create mask where model is not constrained by data
        # get minimum observed quantities
        q1_min_obs = np.amin(np.concatenate(ModelResult1.log_ndfs.data)[:, 0])
        q2_min_obs = np.amin(np.concatenate(ModelResult2.log_ndfs.data)[:, 0])
        ...
        plot_data_with_confidence_intervals(
            ax, q1_q2_relation, self.color, data_masks=data_masks, linewidth=linewidth
        )
        ...
        if scaled_ndf:
            for i, fudge_factor in enumerate(ndf_fudge_factors):
                ...
                # # add (curved) text
                text = f"{fudge_factor}x " + AltModel.quantity_options["ndf_name"]
                CurvedText(
                    x=alt_relations[fudge_factor][1:, 0],
        ...
        if ratio:
            y_label = y_label + "/" + ModelResult1.quantity_options["quantity_name_tex"]
        ax.set_ylabel(y_label, x=0.01)
...
class Plot_q1_q2_relation_evolution(Plot_q1_q2_relation):
    ...
    def _plot(
        ...
        q1_q2_relation = {}
        for z in redshift:
            q1_q2_relation[z] = calculate_q1_q2_relation(
                ModelResult1, ModelResult2, z, log_q1, sigma=sigma, num=num
            )
        ...
        for z in redshift:
            ax.plot(
                q1_q2_relation[z][1][:, 0], q1_q2_relation[z][1][:, 1], label=str(z)
            )
...
class Plot_conditional_ERDF(Plot):
    ...
    def _plot(self, ModelResult, z=0, parameter=None, linewidth=5):
        ...
        if ModelResult.physics_name not in [
            "eddington",
            "eddington_free_ERDF",
            "eddington_changing",
        ]:
            return NotImplementedError(
                "Only implemented for bolometric " "luminosity - Eddington models."
            )
        ...
        lum = [35, 40, 45, 50]
        qlf_cont = ModelResult.calculate_conditional_ERDF(lum, z, parameter, edd_space)
        ...
        for i, l in enumerate(lum):
            ...
            ax.plot(
                ...
                linewidth=linewidth,
                label=r"$\log L_\mathrm{bol}$ = " + str(l) + r" [erg s$^{-1}$]",
            )
...
class Plot_black_hole_mass_distribution(Plot):
    ...
    def _plot(
        ...
        z=0,
        lum=45.2,
        edd_space=np.linspace(-6, 32.13, 10000),
        num=5000,
        sigma=1,
        datapoints=True,
        legend=False,
        ...
        if not np.isscalar(lum):
            raise NotImplementedError(
                "Not yet implemented for multiple " "luminosities. lum must be scalar."
            )
        # sort sigma in reverse order
        sigma = np.sort(make_array(sigma))[::-1]
        ...
        dist_map = ModelResult.calculate_conditional_ERDF(
            lum, z, parameter, edd_space, black_hole_mass_distribution=True
        )
        ...
        plot_data_with_confidence_intervals(
            ax, m_bh_dist, self.color, median=True, linewidth=linewidth
        )
...
class Plot_quantity_density_evolution(Plot):
    ...
    def _plot(
        ...
        if redshift is None:
            redshift = np.arange(
                ModelResult.redshift[0],
                ModelResult.quantity_options["extrapolation_end"] + 1,
            )
        # make list if input is scalar
        redshift = make_list(redshift)

        # calculate quantity densities
        densities = calculate_quantity_density(
            ModelResult,
            redshift,
            sigma=1,
            return_samples=True,
        ...
        cm_obs = LinearSegmentedColormap.from_list(
            "Custom", [washed_out_color_obs, self.color], N=num_samples
        )
        washed_out_color_pred = blend_color(additional_color, 0.2)
        cm_pred = LinearSegmentedColormap.from_list(
            "Custom", [washed_out_color_pred, additional_color], N=num_samples
        )
        ...
        for z in redshift:
            ...
            # create xvalues and add scatter for easier visibility
            x = np.repeat(z, len(sample)) + np.random.normal(
                scale=0.06, size=len(sample)
            )

            cm = cm_obs if z in ModelResult.redshift else cm_pred
            ax.scatter(x, sample, c=color, s=0.1, cmap=cm, rasterized=rasterized)
...
class Plot_stellar_mass_density_evolution(Plot_q1_q2_relation):
    ...
    def _plot(
        ...
        smd_dict, inferred_smd_dict = calculate_stellar_mass_density(
            mstar,
            muv,
            sigma=1,
            return_samples=True,
        ...
        washed_out_color = blend_color(self.color, 0.2)
        cm = LinearSegmentedColormap.from_list(
            "Custom", [washed_out_color, self.color], N=num_samples
        )
        cm_greys = LinearSegmentedColormap.from_list(
            "Custom", ["lightgrey", "grey"], N=num_samples
        )
        ...
        for z in redshift:
            ...
            inferred_smd_sample = inferred_smd_sample[np.isfinite(inferred_smd_sample)]
            ...
            # create xvalues and add scatter for easier visibility
            x1 = np.repeat(z, len(smd_sample)) + np.random.normal(
                scale=0.06, size=len(smd_sample)
            )
            x2 = np.repeat(z, len(inferred_smd_sample)) + np.random.normal(
                scale=0.02, size=len(inferred_smd_sample)
            )
            ...
            ax.scatter(
                x2[:-1],
                inferred_smd_sample[:-1],
                c=inferred_smd_color[:-1],
                s=0.1,
                cmap=cm,
                rasterized=rasterized,
            )
        ...
        # set y lim by calculating percentiles of all available samples
        all_points = np.array([inferred_smd_dict[z] for z in redshift]).flatten()
        y_lims = np.percentile(all_points[np.isfinite(all_points)], [0.05, 99.99])
        ax.set_ylim(*y_lims)
        # add x tick for every redshift
        ax.set_xticks(redshift)
...
class Plot_black_hole_mass_density_evolution(Plot_q1_q2_relation):
    ...
    def _plot(
        ...
        # calculate stellar mass densities
        bhmd_dict, inferred_bhmd_dict = calculate_black_hole_mass_density(
            mbh,
            lbol,
            return_samples=True,
            log_q_space=log_q_space,
        ...
        for z in redshift:
            ...
            inferred_bhmd_sample = inferred_bhmd_sample[
                np.isfinite(inferred_bhmd_sample)
            ]

            bhmd_sample, bhmd_color = sort_by_density(bhmd_sample)
            # create xvalues and add scatter for easier visibility
            x1 = np.repeat(z, len(bhmd_sample)) + np.random.normal(
                scale=0.02, size=len(bhmd_sample)
            )
            ...
            if len(inferred_bhmd_sample) > 0:
                ...
                x2 = np.repeat(z, len(inferred_bhmd_sample)) + np.random.normal(
                    scale=0.06, size=len(inferred_bhmd_sample)
                )

                ax.scatter(
                    x2[:-1],
                    inferred_bhmd_sample[:-1],
                    c=inferred_bhmd_color[:-1],
                    s=0.5,
                    cmap=cm,
                    rasterized=rasterized,
                )
        ...
        if datapoints:
            total_bhmf = np.array([3.1, 5.3]) * 1e5  # Shankar2019
...
class Plot_scatter_ndf(Plot):
    ...
    def _plot(
        ...
        ndf_no_scatter = ModelResult.calculate_log_abundance(
        ...
        washed_out_color = blend_color(self.color, 0.5)
        cm = LinearSegmentedColormap.from_list(
            "Custom", [self.color, washed_out_color], N=len(scatter_parameter)
        )

        # plot ndf without scatter
        ax.plot(
            quantity_range, ndf_no_scatter, label="No scatter", color="grey", zorder=0
        )
...
class Plot_q1_q2_distribution_with_scatter(Plot_q1_q2_relation):
    ...
    def _plot(
        ...
        log_q2_value,
        redshift=0,
        log_q1=np.linspace(8.51, 10.41, 500),
        scatter_1=0.05,
        scatter_2=0.25,
        skew=-40,
    ):

        if ModelResult1.parameter.is_None() or ModelResult2.parameter.is_None():
            raise AttributeError("best fit parameter " "have not been calculated.")
        ...
        JointDistribution1 = Joint_distribution(ModelResult1, "lognormal", scatter_1)
        JointDistribution2 = Joint_distribution(ModelResult2, "lognormal", scatter_2)

        JointDistribution2_skew = Joint_distribution(
            ModelResult2,
            "skewlognormal",
            scatter_parameter=scatter_2,
            skew_parameter=skew,
        )
        # calculate conditional probabilities
        log_q1_probabilities = []
        for Distribution2 in [JointDistribution2, JointDistribution2_skew]:
            log_q1_probabilities.append(
                calculate_q1_q2_conditional_pdf(
                    JointDistribution1,
                    Distribution2,
                    log_q1,
                    log_q2_value,
        ...
        no_scatter_value = calculate_q1_q2_relation(
            ModelResult2, ModelResult1, redshift, log_q2_value
        )[1][0, 1]
