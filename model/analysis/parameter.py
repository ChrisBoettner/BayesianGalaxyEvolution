...
################ MAIN FUNCTIONS ###############################################
def tabulate_parameter(models, precision=2, caption="", use_best_fit=False, **kwargs):
    """
    ...
    argument. If use_best_fit is True, use best fit parameter (without uncertainties).
    Further kwargs are passed to calculate_parameter_with_uncertainties.
    """
    models = make_list(models)

    # pre-load dataframe
    parameter_dict = calculate_parameter_with_uncertainties(
        models, use_best_fit=use_best_fit, **kwargs
    )
    redshift = list(parameter_dict.keys())

    parameter_num = len(parameter_dict[redshift[0]])
    formatted_DataFrame = pd.DataFrame(index=redshift, columns=range(parameter_num))

    for p in range(parameter_num):
        for z in redshift:
            if use_best_fit:
                param = parameter_dict[z][p]
                param = np.around(param, precision)
                string = str(param[0]) if np.isfinite(param[0]) else ""
    ...
    for model in models:
        if not model.fixed_m_c:
            header.insert(
                idx_counter,
                r"$\log M_\mathrm{c}^"
                + model.quantity_options["quantity_subscript"]
                + r"$",
            )
            idx_counter += 1
    ...
    latex_table = formatted_DataFrame.to_latex(
        index=False, escape=False, column_format=column_format, caption=caption
    )

    return latex_table


def calculate_parameter_with_uncertainties(
    models, marginalise=True, redshift=np.arange(11), sigma_equiv=2, use_best_fit=False
):
    """
    ...
    nuisance parameter. If use_best_fit is True, use best fit parameter (without
    uncertainties).
    """
    models = make_list(models)
    ...
    for z in redshift:
        ...
        for model in models:
            ...
            if z in model.redshift:
                if use_best_fit:
                    best_fit_parameter = model.parameter.at_z(z)
                    model_par = np.full((len(best_fit_parameter), 3), np.nan)
                    if marginalise and z >= model.quantity_options["marg_z"]:
                        model_par[model.quantity_options["marg_keep"], 0] = (
                            best_fit_parameter[model.quantity_options["marg_keep"]]
                        )
                    else:
                        model_par[:, 0] = best_fit_parameter

                else:
                    distribution = model.distribution.at_z(z)
                    if marginalise and z >= model.quantity_options["marg_z"]:
                        distribution = distribution[
                            :, model.quantity_options["marg_keep"]
                        ]
                        marg_flag = True

                    # calculate percentiles
                    model_par = calculate_percentiles(
                        distribution, sigma_equiv=sigma_equiv, mode="uncertainties"
                    ).T
