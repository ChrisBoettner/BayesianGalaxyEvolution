...
def calculate_q1_q2_conditional_pdf(
    Jointdist1, Jointdist2, log_q1, log_q2, parameter1, parameter2, z, **kwargs
):
    """
    ...
    **kwargs : dict
    ...
    log_m_h_space = Jointdist1.make_log_m_h_space(log_q1, z, parameter1, **kwargs)
    ...
    conditional_q1 = Jointdist1.quantity_conditional_density(
        log_q1, log_m_h_space, z, parameter1
    )
    joint_q2 = Jointdist2.joint_number_density(log_q2, log_m_h_space, z, parameter2)
    marginal_q2 = Jointdist2.quantity_marginal_density(log_q2, z, parameter2)
...
class Joint_distribution:
    def __init__(
        self, model, scatter_name, scatter_parameter=None, skew_parameter=None
    ):
        """
    ...
    def joint_number_density(
        ...
        scatter = self.quantity_conditional_density(
            log_quantity, log_halo_mass, z, parameter, scatter_parameter, skew_parameter
        )
        hmf = self.halo_mass_marginal_density(log_halo_mass, z)
        return scatter * hmf

    def quantity_marginal_density(
        self,
        log_quantity,
        z,
        parameter,
        ...
        **kwargs
        ...
        log_m_h_space = self.make_log_m_h_space(log_quantity, z, parameter, **kwargs)
        # calculate ndf values at these m_h points and input quantity values
        number_densities = self.joint_number_density(
            log_quantity, log_m_h_space, z, parameter, scatter_parameter, skew_parameter
        )
        # integrate over sample
        marginal_density = trapezoid(y=number_densities, x=log_m_h_space, axis=0)
        return marginal_density

    def quantity_conditional_density(
        self,
        log_quantity,
        log_halo_mass,
        z,
        ...
        if self.scatter_name == "lognormal":
            pdf_values = norm.pdf(x=log_quantity, loc=log_Q, scale=scatter_parameter)
        elif self.scatter_name == "skewlognormal":
            if skew_parameter is None:
                raise ValueError(
                    "skew_parameter must be specified for " "skewlognormal model."
                )
            # correction factor to create the desired central value
            correction_factor = self._skewlognormal_correction_factor(
                scatter_parameter, skew_parameter, skew_correction
            )
            ...
            # linear space
            pdf_values = norm.pdf(
                x=10**log_quantity, loc=10**log_Q, scale=scatter_parameter * 10**log_Q
            )
        elif self.scatter_name == "cauchy":
            # linear space
            pdf_values = cauchy.pdf(
                x=10**log_quantity, loc=10**log_Q, scale=scatter_parameter * 10**log_Q
            )
        else:
    ...
    def halo_mass_marginal_density(self, log_halo_mass, z):
        ...
        """
        hmf_values = np.power(10, self.model.calculate_log_hmf(log_halo_mass, z))
        return hmf_values

    def halo_mass_conditional_density(
        self,
        log_halo_mass,
        log_quantity,
        z,
        ...
        **kwargs are passed to quantity_marginal_density.
        ...
        joint_dens = self.joint_number_density(
            self,
            log_quantity,
            log_halo_mass,
            z,
        ...
        quantity_marg_dens = self.quantity_marginal_density(
            log_quantity, z, parameter, scatter_parameter, skew_parameter, **kwargs
        )
    ...
    def make_log_m_h_space(
        self, log_quantity, z, parameter, num=int(1e3), epsilon=1e-8
    ):
        """
        ...
        lower_part = np.geomspace(
            epsilon, log_m_epsilon - self.model.log_min_halo_mass, num
        )
        upper_part = np.geomspace(
            epsilon, self.model.log_max_halo_mass - log_m_epsilon, num
        )
        ...
        try:
            points = np.ceil(
                np.nanmax(distance[samp_dens > 0] / samp_dens[samp_dens > 0])
            ).astype(int)
        except ValueError:
            points = 0
        ...
        if points >= num:
            warnings.warn(
                "Number of points created for middle section "
                "exceeds specified number of points in "
                "make_log_m_h_space."
            )
        middle_part = np.linspace(lower_part[-1], upper_part[0], points, endpoint=False)

        # join spaces (sorted in order)
        log_m_h_space = np.concatenate([lower_part, middle_part[1:], upper_part])
    ...
    def _skewlognormal_correction_factor(
        self, scatter_parameter=None, skew_parameter=None, statistic="median"
    ):
        """
        ...
        if statistic == "mean":
            ...
            term_3 = (
                np.sign(skew_parameter) / 2 * np.exp(2 * np.pi / np.abs(skew_parameter))
            )
            correction_factor = skew_parameter * (mu_z - term_2 - term_3)
        else:
            raise NotImplementedError(
                "statistic must be either mean, median " " or mode"
            )
