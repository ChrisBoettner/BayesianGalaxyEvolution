...
class ModelResult:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        initial_guess : dict, optional
            Can give an optional dict of initial guesses for the parameter fit,
            of form {z:initial_guess}.
        log_m_c : dict, optional
            An optional dictonary with the log_m_c values, of shape {z: log_m_c}
        **kwargs : dict
        ...
        self.log_hmfs = Redshift_dict(
            hmf_funcs, extrapolate=False  # callable
        )  # functions

        self.log_turnover_halo_mass = Redshift_dict(
            turnover_halo_mass, extrapolate=False
        )
        self.total_halo_number = Redshift_dict(halo_number, extrapolate=False)
        ...
        if ndf_fudge_factor:
            print(
                "Careful: Number densities adjusted by a factor of "
                f"{ndf_fudge_factor}."
            )
            for key in self.log_ndfs.dict.keys():
                self.log_ndfs.dict[key][:, 1] = self.log_ndfs.dict[key][
                    :, 1
        ...
        self.initial_guess = (
            {z: None for z in self.redshift} if initial_guess is None else initial_guess
        )

        # load options related to the quantity
        self.quantity_options = get_quantity_specifics(self.quantity_name)
        self.log_m_c = self.quantity_options["log_m_c"] if log_m_c is None else log_m_c
        if physics_name not in self.quantity_options["physics_models"]:
            raise NotImplementedError(
                "Physics model not implemented for this " "quantity."
            )

        # location where files will be saved or loaded from
        self.directory = (
            system_path() + self.quantity_name + "/" + self.physics_name + "/"
        )
        ...
        if saving_mode == "loading":
            try:
                ...
                self.parameter = Redshift_dict(parameter, extrapolate=extrapolate)
                # automatically determine if m_c is fixed or not
                if (
                    len(parameter[self.redshift[0]])
                    ...
                    warnings.warn(
                        "Could not determine if m_c is fixed based "
                        "on parameter, might have to choose "
                        "manually."
                    )
            except FileNotFoundError:
                warnings.warn(
                    "Could not load best fit parameter. "
        ...
        if self.physics_name == "none":
            ...
            self._plot_parameter("C3", "v", "-", "Stellar + Black Hole Feedback")
        elif self.physics_name == "changing":
            feedback_change_z = self.quantity_options["feedback_change_z"]
            max_z = 10
            self._plot_parameter(
                ["C3"] * feedback_change_z
                + ["lightgrey"] * (max_z + 1 - feedback_change_z),
                "o",
                "-",
            ...
            self._plot_parameter("C3", "^", "-", "BH Growth Model")
        elif self.physics_name == "eddington":
            self._plot_parameter("C3", "v", "-", "Bolometric Luminosity Model")
            ...
            max_z = 7
            self._plot_parameter(
                ["C3"] * feedback_change_z + ["grey"] * (max_z + 1 - feedback_change_z),
                "o",
                "-",
    ...
    def fit_model(self, redshifts, fixed_m_c=True, **kwargs):
        ...
        **kwargs : dict
        ...
        custom_bar_flag = (
            self.progress
            and self.fitting_method == "mcmc"
            and not (self.saving_mode == "loading")
        ...
        for z in PBar(redshifts):
            ...
            if not custom_bar_flag:
                model_details = (
                    self.quantity_options["ndf_name"]
                    + " - "
                    + "physics("
                    + self.physics_name
                    + ") - "
            ...
            if self.prior_name == "uniform":
                prior, bounds = mcmc_fitting.uniform_prior(self, posterior_samp, bounds)
            ...
            if self.fitting_method != "mcmc":
                parameter, posterior_samp = leastsq_fitting.lsq_fit(
                    self,
                    self.fitting_method,
                    initial_guess=self.initial_guess[z],
                )
            else:
                parameter, posterior_samp = mcmc_fitting.mcmc_fit(
                    self,
                    prior,
                    saving_mode=self.saving_mode,
    ...
    def calculate_log_abundance(
        ...
        **kwargs: dict, optional
        ...
        if scatter_name == "delta":
            ...
            if scatter_parameter is None:
                raise ValueError(
                    "scatter_parameter must be specific if "
    ...
    def draw_parameter_sample(
        self, z, num=1, extrapolate_if_possible=False, loop_limit=None
    ):
        """
        ...
        if self.distribution.is_None():
            raise AttributeError(
                "distribution dictonary is empty. Probably" " wasn't calculated."
            )
        ...
        # else linearly extrapolate to higher redshift
            ...
            extrapolation_z = extrapolation_z[extrapolation_z < np.amax(self.redshift)]
            if not np.isin(extrapolation_z, self.redshift).all():
                raise ValueError(
                    "Redshift range used for extrapolation not " "in redshift of model."
                )
            if not extrapolation_z.any():
                raise ValueError(
                    "Extrapolation start point below " "highest redshift in model."
                )
            ...
            while len(parameter_sample) < num:
                ...
                fit = linear_model.fit(
                    X=extrapolation_z[:, np.newaxis], y=np.array(samples)
                )
                ...
                if loop_counter >= loop_limit:
                    raise RuntimeError(
                        "While loop exceeded loop limit. It "
    ...
    def calculate_quantity_distribution(self, log_halo_mass, z, num=int(1e4), **kwargs):
        """
        ...
        for p in parameter_sample:
            log_quantity_dist.append(
                self.physics_model.at_z(z).calculate_log_quantity(log_halo_mass, *p)
            )

        # conversion between magnitude/sfr and luminosity if needed
        log_quantity_dist = self.unit_conversion(log_quantity_dist, self.lum_conversion)
        return np.array(log_quantity_dist)

    def calculate_halo_mass_distribution(self, log_quantity, z, num=int(1e5), **kwargs):
        """
        ...
        for p in parameter_sample:
            log_halo_mass_dist.append(
                self.physics_model.at_z(z).calculate_log_halo_mass(log_quantity, *p)
            )
        return np.array(log_halo_mass_dist)

    def calculate_abundance_distribution(self, log_quantity, z, num=int(1e5), **kwargs):
        """
        ...
        log_abundance_dist = []
        for p in parameter_sample:
            log_abundance_dist.append(self.calculate_log_abundance(log_quantity, z, p))
    ...
    def calculate_ndf(self, z, parameter, quantity_range=None, hmf_z=None):
        ...
        ndf = self.calculate_log_abundance(quantity_range, z, parameter, hmf_z=hmf_z)
    ...
    def get_ndf_sample(self, z, num=100, quantity_range=None, hmf_z=None, **kwargs):
        """
        ...
        ndf_sample = []
        for n in range(num):
            ndf = self.calculate_ndf(
                z, parameter_sample[n], quantity_range=quantity_range, hmf_z=hmf_z
            )
            ndf_sample.append(np.array(ndf).T)
        return ndf_sample

    def calculate_quantity_density(
        self, z, parameter, num=500, log_q_space=None, number_density=False, **kwargs
    ):
        """
        ...
        if log_q_space is None:
            log_m_h_space = np.linspace(
                self.log_min_halo_mass, self.log_max_halo_mass, num=num
            )
        ...
        log_phi = self.calculate_log_abundance(log_q_space, z, parameter, **kwargs)
        ...
        quantity_density = trapezoid(y=10**log_y, x=log_q_space)
        return quantity_density

    def calculate_feedback_regimes(
        self,
        z,
        parameter=None,
        log_epsilon=-1,
        ...
        **kwargs,
        ...
        if self.quantity_name not in ["mstar", "Muv"]:
            raise NotImplementedError(
                "calculate_feedback_regimes not yet "
        ...
        if parameter is None:
            ...
            for p in parameter_sample:
                regime_values.append(
                    self.physics_model.at_z(z)._calculate_feedback_regimes(
        ...
        # conversion between magnitude/sfr and luminosity if needed
        if output == "quantity":
            regime_values = self.unit_conversion(regime_values, self.lum_conversion)
    ...
    def _add_physics_model(self, z, fixed_m_c=True):
        ...
        if self.physics_name in ["none", "stellar", "stellar_blackhole", "quasar"]:
    ...
    def _experimental_log_abundance_with_scatter(
        ...
        **kwargs: dict, optional
        ...
        if hmf_z != z:
            raise NotImplementedError(
                "hmf_z not implemented for calculation " "with scatter."
            )
        log_quantity = make_array(log_quantity)

        distribution = Joint_distribution(
            self, scatter_name, scatter_parameter, skew_parameter
        )
        phi = distribution.quantity_marginal_density(
            log_quantity, z, parameter, **kwargs
        )
        return np.log10(phi)
...
class ModelResult_QLF(ModelResult):
    ...
    def calculate_log_abundance(self, log_L, z, parameter, num=100, hmf_z=None):
        """
        Calculate (log of) value (phi) of modelled number density function by
        ...
        for L in log_L:
            ...
            eddington_ratio_space = self.make_log_eddington_ratio_space(
                L, z, parameter, num=num
            )
            # calculate QLF contribution at these redhifts
            log_qlf_contribution = self.calculate_log_QLF_contribution(
                eddington_ratio_space, L, z, parameter, hmf_z=hmf_z
            )
            # integrate over the contributions and append to list
            phi.append(
                trapezoid(np.power(10, log_qlf_contribution), eddington_ratio_space)
            )
    ...
    def calculate_log_QLF_contribution(
        self, log_eddington_ratio, log_L, z, parameter, hmf_z=None
    ):
        """
        ...
        log_phi = self.calculate_phi_contribution(
            log_eddington_ratio, log_L, z, parameter, hmf_z=hmf_z
        )
        # calculate contribution from ERDF
        log_erdf = self.calculate_ERDF_contribution(log_eddington_ratio, z, parameter)
        # put it together
        ...
        if self.physics_model.at_z(z).parameter[1] <= (
            ...
            raise ValueError(
                "Slope of ERDF smaller than 1+(slope of HMF/"
                "eta). QLF integral will not converge."
            )
        return log_qlf_contribution

    def calculate_quantity_distribution(
        self, log_halo_mass, z, log_eddington_ratio=None, num=int(2e3), **kwargs
    ):
        """
        At a given redshift, calculate distribution of observable quantity
        ...
        if log_eddington_ratio is not None:
            ...
            if self.physics_model.at_z(z).name == "eddington":
                ...
                for i, p in enumerate(parameter_sample[:, -2:]):
                    log_edd_ratio[i] = self.physics_model.at_z(z).draw_eddington_ratio(
                        *p
                    )
    ...
    def calculate_halo_mass_distribution(
        self, log_L, z, log_eddington_ratio=None, num=int(2e3), **kwargs
    ):
        """
        At a given redshift, calculate distribution of halo mass for a given
        ...
        if log_eddington_ratio is not None:
            ...
            if self.physics_model.at_z(z).name == "eddington":
                log_edd_ratio = np.repeat(
                    self.physics_model.at_z(z).draw_eddington_ratio(), num
                )
                parameter = parameter_sample  # should only contain the two
                # parameter
            elif self.physics_model.at_z(z).name in [
                ...
                for p in parameter_sample[:, -2:]:
                    log_edd_ratio = np.append(
                        log_edd_ratio,
                        self.physics_model.at_z(z).draw_eddington_ratio(*p),
                    )
                parameter = parameter_sample[:, :-2]
            else:
                raise NameError("physics_name not known.")
    ...
    def calculate_phi_contribution(
        self, log_eddington_ratio, log_L, z, parameter, hmf_z=None
    ):
        """
    ...
    def calculate_ERDF_contribution(self, log_eddington_ratio, z, parameter):
        """
        ...
        # physics function with these parameter.
            ...
            log_erdf = self.physics_model.at_z(z).calculate_log_erdf(
                log_eddington_ratio, *parameter[-2:]
            )
        else:
            raise NameError("physics_name not known.")
        return log_erdf

    def calculate_conditional_ERDF(
        self,
        log_L,
        z,
        parameter,
        ...
        for l in log_L:
            ...
            qlf_con = np.power(
                10,
                self.calculate_log_QLF_contribution(
                    eddington_ratio_space, l, z, parameter
                ),
            )
            # the contribution has to be normalised
            normalisation = trapezoid(qlf_con, eddington_ratio_space)
            conditional_erdf = np.array(
                [eddington_ratio_space, qlf_con / normalisation]
            ).T
            ...
            if black_hole_mass_distribution:
                ...
                # black hole masses
                log_m_bhs = l - conditional_erdf[:, 0] - self.log_eddington_constant
    ...
    def calculate_expected_log_black_hole_mass_from_ERDF(self, log_L, z, parameter):
        """
        ...
        black_hole_mass_distributions = self.calculate_conditional_ERDF(
            log_L, z, parameter, black_hole_mass_distribution=True
        )
    ...
    def calculate_quantity_density(
        self, z, parameter, num=200, log_q_space=None, number_density=False, **kwargs
    ):
        """
        Calculate the total quantity density at redshift z by integrating
        ...
        if log_q_space is None:
            log_q_space = np.linspace(
                38, 55, num=num  # L_edd for one solar mass black hole
            )
    ...
    def make_log_eddington_ratio_space(self, log_L, z, parameter, log_cut=6, num=100):
        """
        ...
        initial_qlf_points = self.calculate_log_QLF_contribution(
            initial_eddington_space, log_L, z, parameter
        )
        ...
        if relevant_contribution[0] == False and relevant_contribution[-1] == False:
        ...
        # if not, further refine space
            ...
            delta_edd = initial_eddington_space[-1] - initial_eddington_space[-2]
            slope_inverse = delta_edd / delta_qlf_cont

            if slope_inverse > 0:
                raise ValueError(
                    "Slope estimate of large eddington_ratio end "
    ...
    def _add_physics_model(self, z, fixed_m_c=True):
        ...
        if self.physics_name == "none":
            ...
            if z == self.redshift[0]:
                ...
                if self.calibrate:
                    ...
                    try:
                        eddington_erdf_params = self.parameter.at_z(self.redshift[0])[
                            -2:
                        ]
                    except:
                        raise NameError(
                            "Cannot look up MAP estimate for ERDF "
            ...
            if z < feedback_change_z:
                ...
                if self.calibrate:
                    ...
                    eddington_erdf_params = self.parameter.at_z(feedback_change_z - 1)[
                        -2:
                    ]
                else:
                    eddington_erdf_params = None
        ...
        if (ph_name == "eddington") and self.calibrate and (self._initial_erdf is None):
            self._initial_erdf = self.physics_model.at_z(z).calculate_log_erdf(
    ...
    def at_z(self, z):
        ...
        if z not in self.avail_redshift:
            if (z > np.amax(self.avail_redshift)) and self.extrapolate:
                ...
                raise NameError("Redshift not in data and cannot be " "extrapolated.")
        else:
            return self.dict[z]
