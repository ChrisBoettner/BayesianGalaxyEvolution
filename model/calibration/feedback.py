...
def feedback_model(feedback_name, log_m_c, initial_guess, bounds):
    ...
    if feedback_name == 'none':  
        model = NoFeedback(log_m_c, initial_guess[:1],
                           bounds[:,:1])
    elif feedback_name == 'stellar': 
        model = StellarFeedback(log_m_c, initial_guess[:2],
                                bounds[:,:2])
    elif feedback_name == 'stellar_blackhole':  
        model = StellarBlackholeFeedback(log_m_c, initial_guess,
                                         bounds)
    else:
...
class StellarBlackholeFeedback(object):
    def __init__(self, log_m_c, initial_guess, bounds):
        ...
        self._upper_m_h    = 40              # max halo mass (just used to avoid overflows)
    ...
    def calculate_log_halo_mass_alternative(self, log_quantity, log_A, alpha,
        ...
        log_quantity_lookup = StellarBlackholeFeedback.calculate_log_quantity(self,
                                                                              log_m_h_lookup,
                                                                              log_A, 
                                                                              alpha,
                                                                              beta)
