...
def lsq_fit(model, method="least_squares", initial_guess=None):
    """
    ...
    if method == "least_squares":
        ...
        optimization_res = dual_annealing(
            cost_function, bounds=bounds, maxiter=100, args=(model,)
        )
    elif method == "brute":
        optimization_res = brute(cost_function, ranges=bounds, Ns=100, args=(model,))
    else:
...
def cost_function(params, model, out="cost", weighted=True, z=None):
    ...
    if not within_bounds(params, *bounds):
        return 1e30  # if outside of bound, return huge value to for cost func
    ...
    if weighted:
        ...
        data_uncertainties = calculate_data_uncertainties(
            model, relative=relative_weights
        )
