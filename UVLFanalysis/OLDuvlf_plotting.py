...
#madau       = np.load('data/Madau2014UVLF.npz'); madau = {i:madau[j] for i,j in [['0','4'],['1','7'],['2','8'], ['3','9']]}
...
#madau       = group(madau,    range(1,5) ).plot_parameter('black', 'o', 'Cucciati2012')
...
for g in groups:
    for z in g.redshift:
        ax[z-1].errorbar(np.log10(g.data_at_z(z).lum), g.data_at_z(z).phi, [g.data_at_z(z).lower_error,g.data_at_z(z).upper_error],
